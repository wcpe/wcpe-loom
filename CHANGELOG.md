...
- **重映射任务支持 Gradle 构建缓存**：为 `remapJar`、`remapSourcesJar` 及其派生任务启用缓存，并按真实用途声明类路径、映射内容、嵌套 JAR 文件名和客户端条目相对路径输入；相同输入下缺失的输出可从缓存恢复。
- **Manifest 版本参与缓存判断**：将实际写入归档的 Gradle、Loom、Minecraft、Loader、Mixin 等版本统一建模为任务输入，避免版本变化后仍复用旧输出；保留模组显式指定的 Mixin 版本和组，并正确处理可重现构建测试开关。
- **尊重非可重现归档设置**：调用方保留原始文件时间戳或关闭可重现文件顺序时不使用重映射输出缓存，避免恢复不符合归档设置的结果。
- **修复版本导出增量判断**：`exportVersion` 将发布版本登记为输入，切换 `loomPublishVersion` 或 CI 发布版本时重新生成 `version.txt`，同一版本才复用已有输出。

### 验证与 CI

- 新增 `RemapTaskCacheTest`，覆盖类与源码归档的 `FROM-CACHE` 恢复及哈希一致性、配置缓存复用、源码和 Manifest 变化、非可重现归档以及无效映射命名空间。
- 发布工作流在构建和 Maven 发布前执行上述缓存回归；保留现有手动 `Run Tests` 测试矩阵，正式标签发布前要求该工作流在同一提交上通过。
