...
cmd_add_patch() {
	local arg="${1:-}"
	[ -n "${arg}" ] || die "用法: scripts/sync-upstream.sh add-patch <补丁名> | add-patch finish [分支名]"
	if [ "${arg}" = "finish" ]; then cmd_add_patch_finish "${2:-}"; return; fi
...
接下来：
  ...
  3) 提交后切回默认分支，执行： scripts/sync-upstream.sh add-patch finish ${branch}
EOF
}
